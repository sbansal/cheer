class NotificationMailer < ApplicationMailer
  def send_weekly_summary(user_id)
    ...
    @net_worth_stat = @company.stats.find_by(name: Stat::NET_WORTH_STAT)
    @net_worth_change = @net_worth_stat.last_change_data[@period]
    @assets_stat = @company.stats.find_by(name: Stat::ASSETS_STAT)
    @assets_change = @assets_stat.last_change_data[@period]
    @liabilities_stat = @company.stats.find_by(name: Stat::LIABILITIES_STAT)
    @liabilities_change = @liabilities_stat.last_change_data[@period]
    @cash_stat = @company.stats.find_by(name: Stat::CASH_STAT)
    @cash_change = @cash_stat.last_change_data[@period]
    @investments_stat = @company.stats.find_by(name: Stat::INVESTMENTS_STAT)
    @investments_change = @investments_stat.last_change_data[@period]
    @cashflow = @company.cashflow(7.days.ago, Time.zone.now)
