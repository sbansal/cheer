...
RSpec.describe TransactionsFetcher do
  ...
  describe 'with no params' do
    it 'queries transactions for all time' do
      ...
      expect(fetcher.duplicates_count).to eq(1)
    end
    ...
    it 'queries transactions for the past week' do
      ...
      expect(fetcher.duplicates_count).to eq(0)
    end
  ...
  describe 'with duplicates' do
    ...
    it 'queries transactions for all time' do
      fetcher = TransactionsFetcher.call(@company, Stat::ALL, params)
      expect(fetcher.aggregated_transactions.count).to eq(1)
      expect(fetcher.aggregated_transactions.total_spend).to eq(500)
      expect(fetcher.duplicates_count).to eq(1)
    end

    it 'queries transactions for the past week' do
      fetcher = TransactionsFetcher.call(@company, Stat::WEEKLY, params)
      expect(fetcher.aggregated_transactions.count).to eq(0)
      expect(fetcher.aggregated_transactions.total_spend).to eq(0)
      expect(fetcher.duplicates_count).to eq(0)
    end
  ...
  def build_historical_transactions
    ...
    tx = create(:transaction, occured_at: 4.month.ago, amount: 400, user: @user, description: 'Amazon Tips*2HTSK0EL3', bank_account: @bank_account)
    create(:transaction, occured_at: 1.year.ago, amount: 500, user: @user, description: 'Amazon Tips*2HTSK0EL3',
      bank_account: @bank_account, category: @category, duplicate: true, duplicate_transaction_id: tx.id)
  end
end
