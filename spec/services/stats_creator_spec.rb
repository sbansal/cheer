...
RSpec.describe StatsCreator do
  ...
  it 'creates the net worth stat' do
    expect(@user.company.stats.count).to eq(0)
    stat_name = Stat::NET_WORTH_STAT
    stat = StatsCreator.call(@user.company_id, stat_name)
    expect(stat.description).to eq(Stat::SUPPORTED_STATS[stat_name])
    expect(@user.company.stats.count).to eq(1)
  ...
  it 'creates all stats' do
    StatsCreator.call(@user.company_id, 'ALL')
    expect(@user.company.stats.count).to eq(10)
  end
