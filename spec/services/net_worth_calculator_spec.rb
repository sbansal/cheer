...
RSpec.describe NetWorthCalculator do
  ...
  it 'creates historical trend since the beginning of the account creation' do
    ...
    expect(NetWorthCalculator.call(user.account)[:historical_trend_data].keys.first).to eq(1.year.ago.beginning_of_day)
    ...
    create(:balance, current: 10500, bank_account: depository, user: user)
    expect(NetWorthCalculator.call(user.account)[:historical_trend_data].keys.first).to eq(2.year.ago.beginning_of_day)
  end
