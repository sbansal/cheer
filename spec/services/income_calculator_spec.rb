...
RSpec.describe IncomeCalculator do
  ...
  it 'calculates the income historical trend' do
    historical_trend_data = IncomeCalculator.call(@company)[:historical_trend_data]
    expect(historical_trend_data.keys.first).to eq @company.first_transaction_occured_at
    today = Date.today
  ...
  it 'has a current value' do
    expect(IncomeCalculator.call(@company)[:current_value]).to eq(1200)
  end

  it 'calculates the income last change trend' do
    expect(IncomeCalculator.call(@company)[:last_change_data]).to be_empty
  end

  it 'has value over time data' do
    value_over_time_data = IncomeCalculator.call(@company)[:value_over_time_data]
