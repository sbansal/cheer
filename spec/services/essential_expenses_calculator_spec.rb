...
RSpec.describe EssentialExpensesCalculator do
  ...
  it 'calculates the income historical trend' do
    ...
    expect(historical_trend_data[this_month]).to eq(0)
  ...
  it 'has a current value' do
    expect(EssentialExpensesCalculator.call(@user.account)[:current_value]).to eq(1100)
  end

  it 'calculates the income last change trend' do
    expect(EssentialExpensesCalculator.call(@user.account)[:last_change_data]).to be_empty
  end

  it 'has value over time data' do
    value_over_time_data = EssentialExpensesCalculator.call(@user.account)[:value_over_time_data]
    expect(value_over_time_data[Stat::THIS_MONTH]).to eq(0)
    expect(value_over_time_data[Stat::LAST_MONTH]).to eq(100)
    expect(value_over_time_data[Stat::QUARTERLY]).to eq(200)
    expect(value_over_time_data[Stat::YEARLY]).to eq(1100)
    expect(value_over_time_data[Stat::ALL]).to eq(1100)
  end
