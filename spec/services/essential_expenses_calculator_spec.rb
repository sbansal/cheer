...
RSpec.describe EssentialExpensesCalculator do
  ...
  it 'calculates the income historical trend' do
    historical_trend_data = EssentialExpensesCalculator.call(@user.company)[:historical_trend_data]
  ...
  it 'has a current value' do
    expect(EssentialExpensesCalculator.call(@company)[:current_value]).to eq(2000)
  end

  it 'calculates the income last change trend' do
    expect(EssentialExpensesCalculator.call(@company)[:last_change_data]).to be_empty
  end

  it 'has value over time data' do
    value_over_time_data = EssentialExpensesCalculator.call(@company)[:value_over_time_data]
