...
RSpec.describe NonEssentialExpensesCalculator do
  ...
  it 'has a current value' do
    expect(NonEssentialExpensesCalculator.call(@user.account)[:current_value]).to eq(600)
  end

  it 'calculates the income last change trend' do
    expect(NonEssentialExpensesCalculator.call(@user.account)[:last_change_data]).to be_empty
  end

  it 'has value over time data' do
    value_over_time_data = NonEssentialExpensesCalculator.call(@user.account)[:value_over_time_data]
